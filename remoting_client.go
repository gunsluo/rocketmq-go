	...
	for seq, response := range self.responseTable {
		if (response.beginTimestamp + 30) <= time.Now().Unix() {
			...
			if response.invokeCallback != nil {
				...
				logger.Warn("remove time out request %v", response)
			}
		}
	}
			...
			if flag == 0 {
				...
				go func() {
					...
					if ok {
						...
						jsonCmd, err := ffjson.Marshal(cmd)

						if err != nil {
							logger.Error("%s", err)
						}
						logger.Error(string(jsonCmd))
					}
				}()
			}
		}
...
func (self *DefalutRemotingClient) close() error {
	self.connTableLock.Lock()
	self.isClosed = true
	for addr, conn := range self.connTable {
		conn.Close()
		delete(self.connTable, addr)
	}
	self.connTableLock.Unlock()
